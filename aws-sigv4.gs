var AWS = (function () {
    ...
    return {
        ...
        request: function (service, region, action, params, method, payload, headers, uri, options) {
            ...
            if (method.toLowerCase() == "post") {
                request = "https://" + host + encodeURIExcludeSlash(uri);
                query = '';
            } else {
                query = "Action=" + action;
                query = ''
                ...
                console.log(uri)
                request = "https://" + host + encodeURIExcludeSlash(uri) + "?" + query;
                request = "https://" + host + encodeURIExcludeSlash(uri); //+"?"+query;
                console.log(request);
            }
            ...
            var CanonicalString = method + '\n'
                + encodeURIExcludeSlash(encodeURIExcludeSlash(uri)) + '\n'
                + '' + '\n'
                + canonHeaders + '\n'
                + signedHeaders + '\n'
                + hashedPayload;
            ...
            var scope = dateStringShort + "/" + region + "/" + kService + "/aws4_request";
            ...
            var key = getSignatureKey(Crypto, secretKey, dateStringShort, region, kService);
            ...
            // request = "https://bedrock-runtime.us-west-2.amazonaws.com/model/anthropic.claude-3-sonnet-20240229-v1%3A0/invoke"
    ...
    function encodeURIExcludeSlash(str) {
